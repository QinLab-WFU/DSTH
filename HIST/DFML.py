...
class MarginLoss(torch.nn.Module):
    def __init__(self, margin=0.2, nu=0, beta=1.2, nb_classes=100,device=None):
        super(MarginLoss, self).__init__()
        self.margin = margin
        self.n_classes = nb_classes
        self.beta = torch.nn.Parameter(torch.ones(self.n_classes) * beta).to(device)
        # self.beta = beta
        self.nu = nu
        self.device = device

    def distanceweightedsampling(self, batch, labels, lower_cutoff=0.5, upper_cutoff=1.4,feat2=None):
        """
        This methods finds all available triplets in a batch given by the classes provided in labels, and select
        ...
        distances = self.pdist(batch.detach()).clamp(min=lower_cutoff) if feat2==None else self.pdist(batch.detach(),feat2).clamp(min=lower_cutoff)

        positives, negatives = [], []
        labels_visited = []
        anchors = []
        sim_mat = (labels @ labels.T) > 0
        for idx , i in enumerate(range(bs)):
            neg = sim_mat[i]
            pos = (1 - sim_mat[i]).astype(np.bool_)
            q_d_inv = self.inverse_sphere_distances(batch, distances[i], labels, labels[i]) # [tot,anchor]
    ...
    def pdist(self, A,feat2=None):
        ...
        prod = torch.mm(F.normalize(A), F.normalize(A).t()) if feat2 == None else torch.mm(F.normalize(A), F.normalize(feat2).t())
        norm = prod.diag().unsqueeze(1).expand_as(prod)
        res = (norm + norm.t() - 2 * prod).clamp(min=0)
    ...
    def inverse_sphere_distances(self, batch, dist, labels, anchor_label):
        ...
        # if multi-label dataset
        sim_mat = (labels @ anchor_label.T) > 0
        ...
        # [b,cls] @ [cls,] = [b]
        log_q_d_inv[np.where((labels @ anchor_label.T) > 0)[0]] = 0

        q_d_inv = torch.exp(log_q_d_inv - torch.max(log_q_d_inv))  # - max(log) for stability
        # Set sampling probabilities of positives to zero
        q_d_inv[np.where((labels @ anchor_label.T) > 0)[0]] = 0
    ...
    def forward(self, embeddings, labels,feat2=None):
        if isinstance(labels, torch.Tensor): labels = labels.detach().cpu().numpy()
        sampled_triplets = self.distanceweightedsampling(embeddings, labels)

        d_ap, d_an = [], []
        for triplet in sampled_triplets:
            train_triplet = {'Anchor': embeddings[triplet[0], :], 'Positive': embeddings[triplet[1], :],
        ...
        indice = torch.nonzero(labels).to(self.device)
        ...
        # beta = self.beta
        beta = torch.stack([self.beta[i[1]] for i in indice]).to(self.device)
        beta = F.interpolate(beta.unsqueeze(0).unsqueeze(1),size=(embeddings.shape[0],),mode='linear')
        # beta = []
        # for i in range(indice.shape[0]):
        #     temp = []
        #     temp2 = indice[:,0]
        #     temp3 = indice[:,1]
        #     if ( temp2[i]== temp2[i + 1] or temp2[i] == temp2[i - 1]) :
        #         temp.append(self.beta[temp3[i]])
        #     else :
        #         beta.append(torch.vstack(temp).mean())
        d_an = torch.tensor(d_an).to(self.device)
        d_ap = torch.tensor(d_ap).to(self.device)
...
criterion = MarginLoss(device='cuda')
import numpy as np
...
tot_cls = np.eye(100)
indice = np.random.choice(np.arange(32),size=(32,))
