import Triplet.triplet_selection as triplet_selection
...
class TripletLoss(nn.Module):
    ...
    def forward(self, label, source, target):
        ...
        # self.num_last_triplets = torch.sum(mask.int())
        # self.num_last_positive_triplets = num_positive_triplets
        # self.total_triplets += num_positive_triplets

        # Store some values for later visualization.
        # indices = np.where(mask)
        # if len(indices[0]) > 0:
        #     # Get the first selected triplet.
        #     a_index, p_index, n_index = indices[0][0], indices[1][0], indices[2][0]
        #
        #     self.last_triplet = (a_index.item(), p_index.item(), n_index.item())
        #     self.last_label_distances = (
        #         label_distances[a_index, p_index].item(), label_distances[a_index, n_index].item())
        #
        #     feat_dist_ap = feature_distances[a_index, p_index]
        #     feat_dist_an = feature_distances[a_index, n_index]
        #
        #     self.last_feat_distances = (feat_dist_ap, feat_dist_an)
        #     self.last_loss = np.amax([0, feat_dist_ap - feat_dist_an + self.margin])
        #
        # del feature_distances
        # del label_distances
        # del anchor_positive_dist
        # del anchor_negative_dist
        # del valid_triplets
        # del mask

        return torch.sum(triplet_loss) / (num_positive_triplets + 1e-16)