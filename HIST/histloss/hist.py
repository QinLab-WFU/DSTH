...
class CDs2Hg(nn.Module):
    def __init__(self, nb_classes, sz_embed, bata=27, alpha=0.1,device=None):
        super(CDs2Hg, self).__init__()
        # Parameters (means and covariance)
        self.means = nn.Parameter(torch.Tensor(nb_classes, sz_embed).to(device))
        self.log_vars = nn.Parameter(torch.Tensor(nb_classes, sz_embed).to(device))
...
class HGNN(nn.Module):
    def __init__(self, nb_classes, sz_embed, hidden,device):
    ...
    def compute_G(self, H):
        ...
        we = torch.ones(n_edge).to(self.device)
